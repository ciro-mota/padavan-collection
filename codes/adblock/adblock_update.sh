...
wget -U "Mozilla/5.0 (X11; Linux x86_64; rv:89.0) Gecko/20100101 Firefox/89.0" -qO- "https://block.energized.pro/spark/formats/hosts.txt" | awk -vr="$ENDPOINT_IP4" '{sub(/^0.0.0.0/, r)} $0 ~ "^"r' >> "$TMPDIR"
wget -U "Mozilla/5.0 (X11; Linux x86_64; rv:89.0) Gecko/20100101 Firefox/89.0" -qO- "https://block.energized.pro/spark/formats/hosts-ipv6.txt" | awk -vr="$ENDPOINT_IP6" '{sub(/^::/, r)} $0 ~ "^"r' >> "$TMPDIR"
...
egrep -v "^[[:space:]]*$" "$ALIST" | awk '/^[^#]/ {sub(/\r$/,"");print $1}' | grep -vf - "$STGDIR" > "$TARGET"
else
cat "$STGDIR" > "$TARGET"
fi
